...
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}},regionStyle:{initial:{fill:'#B8E186'}},series:{regions:[{attribute:'fill'}]},mapUrlByCode:function(code,multiMap){countryCode=code.toLowerCase();waitToAddMarkers(100);var mapName=countryCode+'_'+multiMap.defaultProjection+'_en';return'/lib/jvectormap/maps/'+countryCode+'-'+multiMap.defaultProjection+'-en.js';}});if(inputJSON.markers){addMarkersToMultiMap(jsonMarkers);}
this.createSlider();};function addMarkersTooltip(currentMap){var jvmMapMain=new jvm.Map({map:mType,backgroundColor:background,container:$('#nada'),onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
...
function addRegionsToMap(key){var isoCode=key.split("_")[0].toUpperCase();console.log(isoCode);$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country==isoCode){if(currentCountry.Regions){var regions=currentCountry.Regions;;console.log(maps.maps[key]);var regionColors={};for(var regionIndex in regions){var currentRegionName=regions[regionIndex].name;regionColors[currentRegionName]="#FF00FF";}
console.log(regionColors);var myCustomColors={'AU-SA':'#4E7387','AU-WA':'#333333','AU-VIC':'#89AFBF','AU-TAS':'#817F8E','AU-QLD':'#344B5E','AU-NSW':'#344B5E','AU-ACT':'#344B5E','AU-NT':'#344B5E'};console.log(maps.maps[key].params.series);maps.series.regions[0].setValues(myCustomColors);}}});}
