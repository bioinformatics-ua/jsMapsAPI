...
var vectorMap;var jsonFilters=[];var minColorMap;var maxColorMap;var mDiv;var mType;var background;var filteredMarkers;var thereAreMarkers=false;var VectorialMap=function(){};VectorialMap.prototype.createMap=function(inputJSON,minRadius,maxRadius,mapDiv,minColor,maxColor,mapType,backgroundColor,dataType){background=backgroundColor;mType=mapType;jsonCountries=[];jsonMarkers=[];mDiv=mapDiv;minColorMap=minColor;maxColorMap=maxColor;if(dataType=='countries'){jsonCountries=readCountriesFromJSON(inputJSON.countries);}
else if(dataType=='markers'){thereAreMarkers=true;jsonMarkers=readMarkersFromJSON(inputJSON);console.log(jsonMarkers);filteredMarkers=jsonMarkers;numMarkers=jsonMarkers.length;}
else{console.error('You must give as input a list of markers or countries!');return;}
auxColors=generateColorsForTheCountries();jQuery.ajax({url:'../tooltip-templates/country_tooltip.html',success:function(result){countryTooltip=result;},async:false});jQuery.ajax({url:'../tooltip-templates/marker_tooltip.html',success:function(result){markerTooltip=result;},async:false});jQuery.ajax({url:'../tooltip-templates/region_tooltip.html',success:function(result){regionTooltip=result;},async:false});map=new jvm.Map({container:$('#'+mapDiv),map:mType,backgroundColor:background,onRegionClick:function(e,code){countryCode=code.toLowerCase();var newMap=countryCode+'_mill_en';switchMap(newMap);},onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}});if(dataType=='markers'){filteredMarkers=jsonMarkers;addMarkersToMap();}
this.createSlider();};window.addEventListener("keydown",checkKeyPressed,false);function checkKeyPressed(e){if(e.keyCode=="37"){$('#'+mDiv).empty();removeTooltip();map=new jvm.Map({container:$('#'+mDiv),map:mType,backgroundColor:background,onRegionClick:function(e,code){countryCode=code.toLowerCase();var newMap=countryCode+'_mill_en';switchMap(newMap);},onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}});if(thereAreMarkers){addMarkersToMap();}}}
function buildCountryTooltip(countryName,selectedCountry){var finalTooltip=countryTooltip;finalTooltip=finalTooltip.replace('name',countryName.html());finalTooltip=finalTooltip.replace('count',selectedCountry.Count);return finalTooltip;}
...
function buildRegionTooltip(region){var finalTooltip=regionTooltip;finalTooltip=finalTooltip.replace('name',region.name);return finalTooltip;}
...
applyMultipleFilters(selectedMultipleFilters,jsonFilters);});$("#filter_box_reset_filters").click(function(){$(".filter-box-dropdown").text("Select a value");$(".filter-box-dropdown").val('Select a value');var colors=[];$.each(jsonCountries,function(index,currentCountry){colors[currentCountry.Country]=currentCountry.Count;});reloadMap(colors);$.each(jsonMarkers,function(index,currentMarker){map.addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});});});var textToAppend='<div id="filters_box">'+'<button id="filter_box_apply_filters" type="button" class="btn btn-primary">Filter</button>'+'<button id="filter_box_reset_filters" type="button" class="btn btn-primary">Reset</button></div>';$('filters-box-enumeration').append(textToAppend);}
var resetFiltersBox=function(){for(var i=0;i<numFilters;i++){$("#fbox"+i).text('');$("#fbox"+i).val('');}
var colors=generateColorsForTheCountries();reloadMap(colors);filteredMarkers=jsonMarkers;addMarkersToMap();};function createFiltersBoxCheckboxes(){$.each(jsonFiltersArray,function(index,currentFilter){var boxID='#box'+(index+1);$('#filterBoxCheckboxes').append('<li id="box'+(index+1)+'" class="dropdown-checkbox-example dropdown-checkbox dropdown"></li>');var tab=[];$.each(currentFilter.Values,function(index,currentValue){tab.push({'id':index+1,'label':currentValue,'isChecked':false});});function p(wat){return'<p>'+JSON.stringify(wat)+'</p>';}
function updateStatus(){var $p=$('p.status').empty();$p.append(p(widget.checked()));}
var name=currentFilter.Name.toLowerCase();name=name.charAt(0).toUpperCase()+name.slice(1);$(boxID).dropdownCheckbox({data:tab,autosearch:true,hideHeader:false,showNbSelected:false,templateButton:'<a class="dropdown-checkbox-toggle" data-toggle="dropdown" href="#">'+name+'<span class="dropdown-checkbox-nbselected"></span><b class="caret"></b>'});widget=$(boxID).data('dropdownCheckbox');$('body').on('change:dropdown-checkbox checked checked:all check:all uncheck:all check:checked uncheck:checked',updateStatus());updateStatus();});var textToAppend='<div id="filters_box">'+'<button id="filter_box_apply_filters" type="button" class="btn btn-primary">Filter</button>'+'<button id="filter_box_reset_filters" type="button" class="btn btn-primary">Reset</button></div>';$('#filterBoxCheckboxes').append(textToAppend);$("#filter_box_apply_filters").click(function(){var jsonObject={};var numFilters=jsonFiltersArray.length;var emptyFilters=0;for(var i=0;i<jsonFiltersArray.length;i++){var currentFilter="#box"+(i+1);var selectedItems=getSelectedItems(currentFilter);var itemsArray=[];var keys=Object.keys(selectedItems);for(var j=0;j<keys.length;j++){itemsArray.push(selectedItems[keys[j]].label);}
if(keys.length>0){jsonObject[jsonFiltersArray[i].Name]=itemsArray.join();}else{emptyFilters++;}}
if(emptyFilters!=numFilters){filter(jsonObject);}});$("#filter_box_reset_filters").click(function(){resetFiltersBox();});}
function getSelectedItems(boxID){return $(boxID).dropdownCheckbox("checked");}
function createFiltersBoxWithEnumeration(jsonFilters){var numFilters=jsonFilters.length;$.each(jsonFilters,function(index,currentFilter){var filterName=currentFilter.Name.toLowerCase();filterName=filterName.charAt(0).toUpperCase()+filterName.slice(1);var buttonId='dropdown'+index+'button';var ulId='dropdown'+index;var toAppend='';toAppend+='<p><b>'+filterName+':</b></p>';toAppend+='<div class="form-group">';toAppend+='<input type="text" class="form-control" id="fbox'+index+'"';var placeholder=currentFilter.Values
toAppend+='placeholder="'+placeholder+'" +>';toAppend+='</div>';$('#filters_box_enumeration').prepend(toAppend);$('#filters_box_enumeration').tooltip({title:"Use this filter box to filter by multiple filters",placement:"bottom"});});$("#filters_box_enum_apply_filters").click(function(){var jsonObject={};var numFilters=jsonFilters.length;var emptyFilters=0;for(var i=0;i<jsonFilters.length;i++){var currentFilter="#fbox"+i;var currentFilterValue=$(currentFilter).val();if(currentFilterValue!==''){jsonObject[jsonFilters[i].Name]=currentFilterValue;}else{emptyFilters++;}}
if(emptyFilters!=numFilters){filter(jsonObject);}});$("#filters_box_enum_reset_filters").click(function(){resetFiltersBox();});}
...
function reloadMap(colors){if(map.params.map==mType){console.log('main map');if(jsonCountries.length>0)
readMinMax(colors);$("#"+mDiv).empty();map=new jvm.Map({map:mType,backgroundColor:background,container:$('#'+mDiv),onRegionClick:function(e,code){countryCode=code.toLowerCase();var newMap=countryCode+'_mill_en';switchMap(newMap);},onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:colors}]}});}else{if(jsonCountries.length>0)
readMinMax(colors);$("#"+mDiv).empty();removeTooltip();switchMap(mapType);}}
function removeTooltip(){$('.jvectormap-tip').remove();}
function switchMap(newMap){$('#'+mDiv).empty();removeTooltip();var regionColors=generateColorsForTheRegions(newMap);map=new jvm.Map({map:newMap,backgroundColor:background,container:$('#'+mDiv),onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,regionName,code){var currentMap=newMap.split('_')[0].toUpperCase()
var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country==currentMap){selectedCountry=currentCountry;return;}});var regionFound=false;var selectedRegion;$.each(map.regions,function(index,currentRegion){if(currentRegion.name==code){console.log('+');selectedRegion=currentRegion;regionFound=true;return;}});if(regionFound){regionName.html(buildRegionTooltip(selectedRegion));}else
regionName.html(regionName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:regionColors}]}});if(thereAreMarkers){addMarkersToMap();}
if(jsonCountries){}}
function addMarkersToMap(){$.each(filteredMarkers,function(index,currentMarker){map.addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});});}
...
reloadMap(finalCountries);filteredMarkers=[];if(markersByFilter.length>0){filteredMarkers=markersByFilter[0];for(var i=0;i<markersByFilter.length-1;i++){filteredMarkers=getMarkersIntersection(filteredMarkers,markersByFilter[i+1]);}}
addMarkersToMap();}
...
function applyMultipleFiltersProgramattically(filtersToApply){var keys=Object.keys(filtersToApply)
...
markersHaveFilter[markerIndex]++;}}while(true)});});$.each(jsonMarkers,function(index,currentMarker){if(markersHaveFilter[index]==numFiltersToApply){map.addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});}});}
var Region=function(regionObject,country){this.Region=regionObject.name;this.Country=country;this.desc='just a region...';};function readRegionsFromJSON(regions,country){var regionsArr=[];$.each(regions,function(index,currentRegion){regionsArr[index]=new Region(currentRegion,country);});return regions;}
function generateColorsForTheRegions(country){var isoCode=country.split("_")[0].toUpperCase();var regionColors=[];$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country==isoCode){if(currentCountry.Regions){var regions=currentCountry.Regions;$.each(regions,function(index,currentRegion){regionColors[currentRegion.name]=100;});}}});return regionColors;}
function addRegionsToMap(key){var isoCode=key.split("_")[0].toUpperCase();$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country==isoCode){if(currentCountry.Regions){var regions=currentCountry.Regions;var palette=['#66C2A5','#FC8D62','#8DA0CB','#E78AC3','#A6D854'];function generateColors(){var colors={},key;colors[maps.region]
for(key in maps.regions){colors[key]=palette[Math.floor(Math.random()*palette.length)];}
return colors;};console.log(generateColors());maps.series.regions[0].setValues(generateColors());}}});}
