...
auxColors=generateColorsForTheCountries();console.log(auxColors);jQuery.ajax({url:'../tooltip-templates/country_tooltip.html',success:function(result){countryTooltip=result;},async:false});jQuery.ajax({url:'../tooltip-templates/marker_tooltip.html',success:function(result){markerTooltip=result;},async:false});maps=new jvm.Map({container:$('#'+mapDiv),map:mType,backgroundColor:background,onRegionClick:function(e,code){countryCode=code.toLowerCase();var newMap=countryCode+'_mill_en';console.log(newMap);switchMap(newMap);},onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}});if(inputJSON.markers){addMarkersToMainMap(jsonMarkers);}
this.createSlider();};window.addEventListener("keydown",checkKeyPressed,false);function checkKeyPressed(e){if(e.keyCode=="37"){$('#'+mDiv).empty();removeTooltip();maps=new jvm.Map({container:$('#'+mDiv),map:mType,backgroundColor:background,onRegionClick:function(e,code){countryCode=code.toLowerCase();var newMap=countryCode+'_mill_en';console.log(newMap);switchMap(newMap);},onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}});addMarkersToMainMap(jsonMarkers);}}
function removeTooltip()
{$('.jvectormap-tip').remove();}
function switchMap(newMap){$('#'+mDiv).empty();removeTooltip();var regionColors=generateColorsForTheRegions(newMap);maps=new jvm.Map({map:newMap,backgroundColor:background,container:$('#'+mDiv),onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:regionColors}]}});if(jsonMarkers){addMarkersToMainMap(jsonMarkers);}
if(jsonCountries)
{}}
...
function addMarkersToMainMap(jsonMarkers){$.each(jsonMarkers,function(index,currentMarker){maps.addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});});}
