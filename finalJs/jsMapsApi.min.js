...
found=true;};if(found){addMarkersToThisMap(key);addMarkersTooltip(key);}
else
...
markersHaveFilter[markerIndex]++;}}while(true)});});$.each(jsonMarkers,function(index,currentMarker){if(markersHaveFilter[index]==numFiltersToApply){map.addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});}});}
var Region=function(regionObject,country){this.Region=regionObject.name;this.Country=country;this.desc='just a region...';console.log(this);};function readRegionsFromJSON(regions,country){var regionsArr=[];$.each(regions,function(index,currentRegion){regionsArr[index]=new Region(currentRegion,country);});return regions;}
