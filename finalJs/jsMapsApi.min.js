...
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}});addMarkersToMap();}}
...
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:regionColors}]}});if(jsonMarkers){addMarkersToMap();}
if(jsonCountries){}}
...
function generateColorsForTheRegions(country){var isoCode=country.split("_")[0].toUpperCase();var regionColors=[];$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country==isoCode){if(currentCountry.Regions){var regions=currentCountry.Regions;$.each(regions,function(index,currentRegion){regionColors[currentRegion.name]=100;});}}});console.log(regionColors);return regionColors;}
