...
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}},regionStyle:{initial:{fill:'#B8E186'}},series:{regions:[{attribute:'fill'}]},mapUrlByCode:function(code,multiMap){var countryCode=code.toLowerCase();var mapName=countryCode+'_'+multiMap.defaultProjection+'_en';waitAndList(500);return'/lib/jvectormap/maps/'+countryCode+'-'+multiMap.defaultProjection+'-en.js';}});if(inputJSON.markers){addMarkersToMultiMap(jsonMarkers);}
this.createSlider();};function waitAndList(waitingTime)
{setTimeout(function(){listMaps();},waitingTime);}
function listMaps(){availableMaps=maps.maps;console.log(availableMaps);$.each(jsonMarkers,function(index,currentMarker){for(var key in availableMaps){maps.maps[key].addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});}});}
