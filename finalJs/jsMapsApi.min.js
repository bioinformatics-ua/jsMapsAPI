...
var auxColors=generateColorsForTheCountries();jQuery.ajax({url:'../tooltip-templates/country_tooltip.html',success:function(result){countryTooltip=result;},async:false});jQuery.ajax({url:'../tooltip-templates/marker_tooltip.html',success:function(result){markerTooltip=result;},async:false});maps=new jvm.Map({container:$('#'+mapDiv),map:mType,backgroundColor:background,onRegionClick:function(e,code){countryCode=code.toLowerCase();var newMap=countryCode+'_mill_en';console.log(newMap);switchMap(newMap);},onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}});if(inputJSON.markers){addMarkersToMultiMap(jsonMarkers);}
this.createSlider();};function switchMap(newMap){var auxColors=generateColorsForTheCountries();$('#'+mDiv).empty();jvmMapMain=new jvm.Map({map:newMap,backgroundColor:background,container:$('#'+mDiv),onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:auxColors}]}});}
function addMarkersTooltip(currentMap){var jvmMapMain=new jvm.Map({map:mType,backgroundColor:background,container:$('#nada'),onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:[]}]}});var onMarkerFunction=maps.maps[mapType].params.onMarkerTipShow;maps.maps[currentMap].params.onMarkerTipShow=jvmMapMain.params.onMarkerTipShow;maps.maps[currentMap].params.series=jvmMapMain.params.series;maps.maps[currentMap].params.onRegionTipShow=jvmMapMain.params.onRegionTipShow;}
function waitToAddMarkers(waitingTime){setTimeout(function(){availableMaps=maps.maps;var found=false;for(var key in availableMaps){if(key==countryCode+'_mill_en')
found=true;};if(found){addMarkersToThisMap(key);addMarkersTooltip(key);addRegionsToMap(key);}
else
waitToAddMarkers(100);},waitingTime);}
function addMarkersToThisMap(currentMap){$.each(jsonMarkers,function(index,currentMarker){maps.maps[currentMap].addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});});}
...
readMinMax(colors);$("#"+mDiv).empty();jvmMapMain=new jvm.Map({map:mType,backgroundColor:background,container:$('#'+mDiv),onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
...
function addMarkersToMultiMap(jsonMarkers){availableMaps=maps.maps;$.each(jsonMarkers,function(index,currentMarker){for(var key in availableMaps){maps.maps[key].addMarker(index,{latLng:[currentMarker.Latitude,currentMarker.Longitude],name:currentMarker.desc,style:{fill:'green',r:mapRange(currentMarker.Count,minCount,maxCount,minRadius,maxRadius)}});}});}
...
function addRegionsToMap(key){var isoCode=key.split("_")[0].toUpperCase();$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country==isoCode){if(currentCountry.Regions){var regions=currentCountry.Regions;var regionColors={};for(var regionIndex in regions){var currentRegionName=regions[regionIndex].name;regionColors[currentRegionName]="#FF00FF";}
var myCustomColors={'AU-SA':'#4E7387','AU-WA':'#333333','AU-VIC':'#89AFBF','AU-TAS':'#817F8E','AU-QLD':'#344B5E','AU-NSW':'#344B5E','AU-ACT':'#344B5E','AU-NT':'#344B5E'};var palette=['#66C2A5','#FC8D62','#8DA0CB','#E78AC3','#A6D854'];generateColors=function(){var colors={},keyAux;for(keyAux in maps.maps[key].regions){colors[keyAux]=palette[Math.floor(Math.random()*palette.length)];}
return colors;},map;var colorsAux=generateColors();console.log(colorsAux);maps.maps[key].series={regions:[{attribute:'fill',values:colorsAux}]};maps.maps[key].onRegionOver=function(e,code){console.log(code);};console.log(maps.maps[key]);}}});}
