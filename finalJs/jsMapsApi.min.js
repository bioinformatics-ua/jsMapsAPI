...
function switchMap(newMap){$('#'+mDiv).empty();removeTooltip();var palette=['#66C2A5','#FC8D62','#8DA0CB','#E78AC3','#A6D854'];function generateColors(){var colors={},key;colors[maps.region]
for(key in maps.regions){colors[key]=palette[Math.floor(Math.random()*palette.length)];}
return colors;};console.log(generateColors());maps=new jvm.Map({map:newMap,backgroundColor:background,container:$('#'+mDiv),onMarkerTipShow:function(e,label,index){var finalTooltip=buildMarkerTooltip(jsonMarkers,index);label.html(finalTooltip);},onRegionTipShow:function(e,countryName,code){var selectedCountry=-1;$.each(jsonCountries,function(index,currentCountry){if(currentCountry.Country===code){selectedCountry=currentCountry;return;}});if(selectedCountry!=-1){var finalTooltip=buildCountryTooltip(countryName,selectedCountry);countryName.html(finalTooltip);}else
countryName.html(countryName.html());},series:{markers:[{scale:[minColorMap,maxColorMap],values:[minCount,maxCount],legend:{vertical:true}}],regions:[{scale:[minColorMap,maxColorMap],attribute:'fill',values:generateColors()}]}});if(jsonMarkers){addMarkersToMainMap(jsonMarkers);}
if(jsonCountries)
{}}
